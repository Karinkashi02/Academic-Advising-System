...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Print Media Query for PDF/Print Output */
        @media print {
            * {
            ...
            .pagination, .card-header .btn, .search-box, .filter-btn, .clear-filter-btn {
                display: none !important;
            }
            ...
            .modal, .btn, .search-btn, .profile-btn, .logout-btn {
                display: none !important;
            }
            ...
            /* Print header with date and advisor info */
            body::before {
                content: "Academic Advising System - Report\AGenerated on " attr(data-date);
                display: block;
        ...
        <div class="main-content">
            ...
            <header class="header">
                ...
                <div class="header-actions">
                    <button class="btn btn-primary" id="exportReportBtn" style="display:none">
                            ...
                            <div class="chart-row">
                                ...
                                <div class="chart-card">
                                    ...
                                    <div class="filter-controls">
                                        <div class="filter-checkbox" id="allStudentsFilter" style="display:none">
                                    ...
                                    <div class="table-container">
                                        <table class="table table-striped">
                                            ...
                                            <tbody>
                                                <tr>
                                                    <td>1</td>
                                                    <td><strong>Math Advising Session</strong></td>
            ...
            <footer>
                © 2025 Advising System — University Academic Services | Report Generated: <span id="reportDate"></span>
            </footer>
        </div>
    </div>

    <script>
        // Data variables that will be populated from API
        let studentData = [];
        let academicStatusData = [];
        let monthlySessionsData = [];
        let sessionTypeData = [];
        let pendingSessionsData = [];
        let currentPendingPage = 0;
        const pendingPerPage = 5;
        
        // Fallback sample data (used when APIs are not available)
        ...
        const SAMPLE_ACADEMIC_STATUS_DATA = [
        ...
        const SAMPLE_MONTHLY_SESSIONS_DATA = [
        ...
        const SAMPLE_SESSION_TYPE_DATA = [
        ...
        // Determine webapp context path ('' if deployed at root)
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Load all report data from APIs
            loadReportData();
        ...
        function loadReportData() {
            loadSummaryStats();
            loadAcademicStatusData();
            loadMonthlySessionsData();
            loadSessionTypesData();
            loadStudentPerformanceData();
        }

        // Load summary statistics
        function loadSummaryStats() {
            fetch(ctx + '/api/advisor/report?type=summary')
                .then(response => response.json())
                .then(data => {
                    if (data.error) {
                        console.warn('API Error:', data.error, '- Using sample data');
                        useSampleSummaryStats();
                        return;
                    }
                    document.getElementById('totalStudents').textContent = data.totalStudents || 0;
                    document.getElementById('graduationRate').textContent = data.graduationRate || '0%';
                    document.getElementById('sessionsCompleted').textContent = data.sessionsCompleted || 0;
                    document.getElementById('avgCGPA').textContent = (data.avgCGPA || 0).toFixed(2);
                })
                .catch(error => {
                    console.warn('API unavailable:', error.message, '- Using sample data');
                    useSampleSummaryStats();
                });
        }
        
        function useSampleSummaryStats() {
            document.getElementById('totalStudents').textContent = '24';
            document.getElementById('graduationRate').textContent = '92.3%';
            document.getElementById('sessionsCompleted').textContent = '47';
            document.getElementById('avgCGPA').textContent = '3.52';
        }

        // Load academic status distribution
        function loadAcademicStatusData() {
            fetch(ctx + '/api/advisor/report?type=academic-status')
                .then(response => response.json())
                .then(data => {
                    if (data.error) {
                        console.warn('API Error:', data.error, '- Using sample data');
                        useSampleAcademicStatus();
                        return;
                    }
                    academicStatusData = data.data || [];
                    createDonutChart('academicDonutChart', academicStatusData);
                    createAcademicLegend('academicLegend', academicStatusData);
                    updateStatusDetails(academicStatusData);
                })
                .catch(error => {
                    console.warn('API unavailable:', error.message, '- Using sample data');
                    useSampleAcademicStatus();
        ...
        function useSampleAcademicStatus() {
            const sampleData = [
                { name: "Dean's List", value: 8, color: "#27ae60" },
                { name: "Pass", value: 14, color: "#3498db" },
                { name: "Probation", value: 1, color: "#f39c12" },
            ...
            academicStatusData = sampleData;
            createDonutChart('academicDonutChart', sampleData);
            createAcademicLegend('academicLegend', sampleData);
            updateStatusDetails(sampleData);
        }

        // Load monthly sessions data
        function loadMonthlySessionsData() {
            fetch(ctx + '/api/advisor/report?type=monthly-sessions')
                .then(response => response.json())
                .then(data => {
                    if (data.error) {
                        console.warn('API Error:', data.error, '- Using sample data');
                        useSampleMonthlySessions();
                        return;
                    }
                    monthlySessionsData = data.data || [];
        ...
        function useSampleMonthlySessions() {
            const sampleData = [
                { month: 'May', value: 5, color: '#3498db' },
                { month: 'Jun', value: 6, color: '#2ecc71' },
                { month: 'Jul', value: 4, color: '#e67e22' },
            ...
            monthlySessionsData = sampleData;
            createMonthlyBarChart('monthlySessionsChart', sampleData);
            updateMonthlyStats();
        }

        // Load session types distribution
        function loadSessionTypesData() {
            fetch(ctx + '/api/advisor/report?type=session-types')
                .then(response => response.json())
                .then(data => {
                    if (data.error) {
                        console.warn('API Error:', data.error, '- Using sample data');
                        useSampleSessionTypes();
                        return;
                    }
                    sessionTypeData = data.data || [];
        ...
        function useSampleSessionTypes() {
            const sampleData = [
                { name: "Academic Advising", value: 24, color: "#3498db" },
                { name: "Career Counseling", value: 12, color: "#2ecc71" },
                { name: "Graduation Planning", value: 8, color: "#e67e22" },
            ...
            sessionTypeData = sampleData;
            createDonutChart('sessionDonutChart', sampleData);
            createSessionLegend('sessionLegend', sampleData);
        }

        // Load student performance data
        function loadStudentPerformanceData() {
            fetch(ctx + '/api/advisor/performance')
                .then(response => response.json())
                .then(data => {
                    if (data.error) {
                        console.warn('API Error:', data.error, '- Using sample data');
                        useSampleStudentData();
                        return;
                    }
        ...
        function useSampleStudentData() {
            ...
            filteredStudentData = [...sampleData];
            currentPage = 1;
            renderPerformanceTablePage();
            updatePagination();
        }
        ...
        function loadPendingSessions() {
            fetch(ctx + '/api/advisor/sessions-report?type=pending')
                .then(response => response.json())
                .then(data => {
                    if (data.error) {
                        console.warn('API Error:', data.error, '- Using empty sessions');
                        updatePendingSessionsTable([]);
                        return;
                    }
                    updatePendingSessionsTable(data.sessions || []);
                })
                .catch(error => {
                    console.warn('API unavailable:', error.message, '- Using empty sessions');
                    updatePendingSessionsTable([]);
                });
        }

        // Update status details based on API data
        ...
        // Update pending sessions table
        function updatePendingSessionsTable(sessions) {
            const tableBody = document.querySelector('#sessionAnalytics table tbody');
            if (!tableBody) return;

            // store sessions and reset page if needed
            pendingSessionsData = Array.isArray(sessions) ? sessions : [];
            if (currentPendingPage < 0) currentPendingPage = 0;
            const total = pendingSessionsData.length;
            if (currentPendingPage > Math.max(0, Math.ceil(total / pendingPerPage) - 1)) currentPendingPage = 0;

            tableBody.innerHTML = '';

            if (pendingSessionsData.length === 0) {
                const row = document.createElement('tr');
                row.innerHTML = '<td colspan="5" style="text-align: center;">No pending sessions</td>';
                tableBody.appendChild(row);
            } else {
                const start = currentPendingPage * pendingPerPage;
                ...
                // render pagination controls for pending sessions
                const existing = document.getElementById('pendingSessionsPagination');
                if (existing) existing.remove();
                const totalPages = Math.ceil(total / pendingPerPage);
                if (totalPages > 1) {
                    ...
                    document.getElementById('pendingPrev').addEventListener('click', function() {
                        if (currentPendingPage > 0) { currentPendingPage--; updatePendingSessionsTable(pendingSessionsData); }
                    });
                    document.getElementById('pendingNext').addEventListener('click', function() {
                        if (currentPendingPage < totalPages - 1) { currentPendingPage++; updatePendingSessionsTable(pendingSessionsData); }
                    });
                }
            }
        }
        ...
        function populatePerformanceTable(data) {
            ...
            data.forEach(student => {
                ...
                } else if (student.status === "At Risk" || student.status === "Probation") {
        ...
        function updateMajorStudentsTable(major, statusFilter = null) {
            ...
            fetch(url)
                ...
                .then(data => {
                    ...
                    let majorStudents = data.students || [];
                    ...
                    // Update stats
                    const totalMajorStudents = majorStudents.length;
                    const majorCGPAValues = majorStudents.map(s => parseFloat(s.cgpa));
                ...
                .catch(error => console.error('Error updating major students table:', error));
        }
